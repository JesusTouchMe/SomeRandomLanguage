...
data class ListAccessNode(val list: Token, val access: Node, val posStart: Position, val posEnd: Position) : Node(posStart, posEnd);
...
data class VarAssignNode(val token: Token, val node: Node?) : Node(token.start, node?.end ?: token.end) {
    override fun toString(): String {
...
data class IfNode(val cases: ArrayList<Triple<Node?, Node?, Boolean>>, val elseCase: Node?) : Node(cases[0].first!!.start, elseCase?.end ?: cases[cases.size - 1].first!!.end);

data class ForNode(val varNameToken: Token, val startNode: Node, val endNode: Node, val stepNode: Node?, val bodyNode: Node, val shouldReturnNull: Boolean) : Node(varNameToken.start, bodyNode.end);

data class WhileNode(val condition: Node, val body: Node, val shouldReturnNull: Boolean) : Node(condition.start, body.end);

data class FunDefNode(val varName: Token?, val argNameTokens: ArrayList<Token>, val bodyNode: Node, val shouldReturnNull: Boolean) : Node(
